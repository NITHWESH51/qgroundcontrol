# Indrones Ground Control Station


*Indrones GCS* (GCS) is an intuitive and powerful ground control station for UAVs.

This is a **customized build of QGroundControl** tailored specifically for **Indrones Solutions Private Limited**, featuring
 full flight control and mission planning for any MAVLink enabled drone, and vehicle setup for ArduPilot(APM) powered UAVs. Instructions for *using Indrones GCS* 
 are provided in the [Custom User Manual](https://docs.google.com/document/d/1e2_xGAvfSnsF4zPajzsl4Ara4wakNxDuoX2wSu4Jn4s/edit?tab=t.0) (you may not need them because the UI is very intuitive!)




###  Key Custom Features

1. 🔒 **Secure Authentication**  
   Only verified users can access the system, ensuring safe operations.

2. 🎨 **Custom Branding**  
   The UI features Indrones-specific colors, logos, and assets to reflect company identity.

3. 🚀 **Custom Splash Screen**  
   A personalized splash screen appears on startup to reflect Indrones' branding.

4. 👥 **Role-Based Login System**  
   - **Operator Login** – For regular drone users with limited access and permissions.  
   - **Engineer/Admin Login** – For advanced users with full control and configuration rights.

---

Feel free to contribute or explore the source code to extend this customized GCS further!


Key Links:
* [Website](http://indrones.com) (indrones.com)
* [Custom User Manual](https://docs.google.com/document/d/1e2_xGAvfSnsF4zPajzsl4Ara4wakNxDuoX2wSu4Jn4s/edit?tab=t.0)
* [User Manual](https://docs.qgroundcontrol.com/en/)
* [Developer Guide](https://dev.qgroundcontrol.com/en/)
* [Discussion/Support](https://docs.qgroundcontrol.com/en/Support/Support.html)
* [Contributing](https://dev.qgroundcontrol.com/en/contribute/)
* [License](https://github.com/mavlink/qgroundcontrol/blob/master/COPYING.md)
